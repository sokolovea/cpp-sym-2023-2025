...
/* нахождение пути*/
//n - размер поля
//row, col - текущие номера обрабатываемой строки и столбца
//lab - само поле
//visited - двумерный массив посещенных клеток (0 - не посещена, 1 - посещена)
//path - двумерный массив длины пути для клеток
//plan - очередь вершин (задаются двумя координатами) для обработки 
void find_path(int n, int row, int col, char** lab, int** visited, int** path, queue<int>& plan)
{
    if(!visited[row][col])
    {
        ...
        //печать всех промежуточных длин путей
        for (int i = 0; i < n; i++)
        {
            for (int j = 0; j < n; j++)
            {
